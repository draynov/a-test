  ...
  useEffect(() => {
    ...
    async function loadEditor() {
      try {
        ...
        const templatesResponse = await fetch("/api/section-b/templates?cardType=TEACHER", { cache: "no-store" });
        const templatesData = (await templatesResponse.json()) as TemplatesApiResponse | { error?: string };

        if (!templatesResponse.ok) {
          throw new Error("error" in templatesData && templatesData.error ? templatesData.error : "Неуспешно зареждане на шаблона.");
        }
      ...
      <header className="overflow-hidden rounded-4xl border border-white/70 bg-white/95 p-6 shadow-[0_24px_80px_-24px_rgba(15,23,42,0.24)]">
        ...
        <div className="mt-2 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="rounded-full bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700">Тип карта: TEACHER</div>
            <Link
              href="/app/shabloni"
        ...
        <section className="space-y-6">
          <div className="overflow-hidden rounded-4xl border border-slate-200 bg-white/95 p-6 shadow-[0_24px_80px_-24px_rgba(15,23,42,0.18)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">Тип карта: TEACHER</div>
            </div>
